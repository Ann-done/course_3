...
class ViewAdapter(context: Context, objects: ArrayList<Result>?) :
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        if (resObject != null) {
            ...
            (convertView.findViewById<View>(R.id.SubjectShortName) as TextView).text = resObject.subjectName
            (convertView.findViewById<View>(R.id.Mark) as TextView).text = resObject.mark.toString()
            (convertView.findViewById<View>(R.id.RightAnswersNum) as TextView).text = resObject.rightAnsNum.toString() + "/" + resObject.quNum.toString()
        }
